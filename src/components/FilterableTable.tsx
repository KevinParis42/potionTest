...
const FilterableTable = () => {
  ...
  const productListReducer = (state: ProductType[], action: Actions) => {
    ...
    return productList.filter(product => x[action.type](product[prop as keyof ProductType].toString(), val))
  }

  const [currentProductList, dispatch] = useReducer(productListReducer, productList)

  const updateTable = () => {
    if (!filterValues.operator || !filterValues.value || !filterValues.property) {
      dispatch({ type: '' })
      return
    }
